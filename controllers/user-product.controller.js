...
exports.findAll = async(req,res)=>{
     ...
     try{
        ...
        console.log("Problem in reading use's products")
    }
}
...
exports.addProduct = async(req,res) =>{
    ...
    try{
        ...
        res.status(200).json({status: true, data: result})
        console.log("Success in finding user's products ", username)
    }catch(err){
        res.status(400).json({status:false, data: err})
...
exports.updateProduct = async(req,res)=>{
    ...
    try{
        ...
        res.status(200).json({status:true, data: result})
        console.log("Success in updating user's products")
    }catch (err){
        res.status(400).json({status:false, data: err})
        console.log("Problem in updating user's products ", username)
    }
}
...
exports.deleteProduct = async (req,res)=>{
    ...
    try{
        const result = await User.updateOne(
            ...
            res.status(200).json({status:true, data: result})
            console.log("Success in deleting user's products")    
    }catch (err){
        res.status(400).json({status:false, data: err})
        console.log("Problem in deleting user's products ", username)
    }
}
